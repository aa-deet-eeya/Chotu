...
const googleapi = process.env.token;
...
client.on("message", (msg) => {
  if (msg.content === "--help") {
    const help = new discord.MessageEmbed()
      ...
      .addFields(
        {
          name: "--start alternate commands",
          value:
            "1. chotu server chalu krr\n2. server chala \n3. chotu server chala \n4. server chalu karr\n\n",
        },
        {
          name: "--stop alternate commands",
          value: "1. chotu chal phut abb \n2. server bandd kr",
        }
      );
...
client.on("message", (msg) => {
  if (
    msg.content === "chotu server chalu krr" ||
    msg.content === "server chala" ||
    msg.content === "chotu server chala" ||
    msg.content === "server chalu karr"
  ) {
    if (msg.member.roles.cache.has("707679480591810611")) {
      msg.reply("starting the server ji");
      shell.exec(__dirname + "/scripts/start.sh", {
        silent: true,
        async: true,
      });
    } else {
    ...
    if (!(msg.content === "yt." || msg.content === "yt. ")) {
      ...
      msg.channel.send(
        new discord.MessageEmbed()
          .setColor("#0099ff")
          .setTitle("Error")
          .setDescription("Input some valid search term")
      );
    }
  }
});
